    ...
    def train(self, output_dir='EX-0'):
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            for i, key in enumerate(train_keys):
                ...
                for o in range(0, len(seq)-4):
                    arr[o] = np.absolute(arr[o+1] - arr[o]) + np.absolute(arr[o+2] - arr[o]) + np.absolute(arr[o+4] - arr[o])
                

                
                seq = torch.from_numpy(seq).unsqueeze(0)
                mn = torch.from_numpy(arr).unsqueeze(0)
                ...
                if self.hps.use_cuda:
                    seq, target, mn = seq.float().cuda(), target.float().cuda(), mn.float().cuda()

                seq_len = seq.shape[1]
                y, _ = self.model(seq,seq_len, mn)
                loss_att = 0
    ...
    def eval(self, keys, results_filename=None):
        ...
        with torch.no_grad():
		    ...
		    #print(arr[i])
                ...
                if self.hps.use_cuda:
                    seq, mn= seq.float().cuda(), mn.float().cuda()

                y, att_vec = self.model(seq, seq.shape[1], mn)
